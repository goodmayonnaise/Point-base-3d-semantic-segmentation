...
class KNN(nn.Module):
  def __init__(self, knn=5, search=5, sigma=1.0, cutoff=1.0, nclasses=20):
    super().__init__()
    print("*"*80)
  ...
  def forward(self, proj_range, unproj_range, proj_argmax, px, py):
    ...
    proj_unfold_k_rang = F.unfold(proj_range[None, None, ...], kernel_size=(self.search, self.search), padding=(pad, pad)) # max 20387. min 0. 

    # index with px, py to get ALL the pcld points
    idx_list = (py * W + px).type(torch.long)                   
    ...
    # unproj_unfold_k_rang[unproj_unfold_k_rang < 0] = float("inf") # error
    unproj_unfold_k_rang[unproj_unfold_k_rang == 0] = float("inf") # error
                                                                  # 1 25 19960 max min same 
