  ...
  def do_range_projection(self):
    ...
    if self.crop_size is not None:
      return {'range':self.proj_range[self.crop_size:], 
              ...
              # 'px':self.points[self.keep_idx,0][order], 'py':self.points[self.keep_idx,1][order],
              'px':proj_x, 'py':proj_y,
              # 'keep_idx':self.keep_idx_img_pts
              }
  ...
  def do_label_projection(self):
    ...
    if self.crop_size:
      return {'label': self.proj_sem_label[self.crop_size:],
              ...
              # 'label_3d':np.array([self.learning_map[i] for i in self.sem_label])}
