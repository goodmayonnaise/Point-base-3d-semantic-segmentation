...
from data_loader.laserscan_360_knn import LaserScan, SemLaserScan
import torch
...
class SemanticKITTI(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.front == 360:
            ...
            rdms = torch.zeros([len(self.thetas), 3, *self.input_shape])
            labels = torch.zeros([len(self.thetas), *self.input_shape])
            labels_c = torch.zeros([len(self.thetas), 3, *self.input_shape])

            pxs = torch.zeros([len(self.thetas), 1500000])
            pys = torch.zeros([len(self.thetas), 1500000])
            unproj_ranges = torch.zeros([len(self.thetas), 1500000])
            labels_3d = torch.zeros([len(self.thetas), 1500000])
            proj_ranges = torch.zeros([len(self.thetas), *self.input_shape])
            npoints = torch.zeros([len(self.thetas),])
            ...
            for idx, theta in enumerate(self.thetas):
                ...
                px, py = torch.FloatTensor(x['px']), torch.FloatTensor(x['py'])
                proj_range = torch.FloatTensor(depth)
                unproj_range = torch.FloatTensor(x['unproj_range'])
                label_3d = torch.FloatTensor(self.replace_with_dict(y['label_3d']))
                npoint = px.size(0)

                rdms[idx], labels[idx], labels_c[idx] = rdm, label, label_c
                pxs[idx][:npoint], pys[idx][:npoint], unproj_ranges[idx][:npoint], labels_3d[idx][:npoint],  = px, py, unproj_range, label_3d
                npoints[idx], proj_ranges[idx] = npoint, proj_range

            return {'rdm': rdms, 'label': labels, 'img':img, 'thetas':self.thetas, 'label_c':labels_c,
                    'label_3d':labels_3d, 'proj_range':proj_ranges, 'unproj_range':unproj_ranges, 'px':pxs, 'py':pys, 'npoints':npoints}
...
if __name__ == "__main__":
    ...
    dataset = SemanticKITTI('/vit-adapter-kitti/data/semantic_kitti/kitti/dataset/sequences', shape=(384, 1248), nclasses=20, 
                            mode='valid', front=True, split=False, crop_size=128)
